...
{
	string hexstr = "";
	//string* hexstr = new string(); //造成内存泄露
...
BOOL GetGeneralizedTimeStamp(
	...
	//TimeStamp = SystemTimeToTime_t(&lst) //by wj
...
BOOL GetSignerSignatureInfo(
	...
	DWORD pLength = CertGetPublicKeyLength(X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, &pCertInfo->SubjectPublicKeyInfo);
	CertNode.SubjectPublicKeyLength = pLength;
	CRYPT_BIT_BLOB  PublicKey = pCertInfo->SubjectPublicKeyInfo.PublicKey;
	CertNode.SubjectPublicKey = byteToHexStr(PublicKey.pbData, PublicKey.cbData);
