...
string output_target(double value);
string output(int index, double value);
char* readline(FILE *input);
int clean_up(FILE *fp_restore, FILE *fp, const char *msg);
int clean_up(FILE *fp_restore);
...
#define SKIP_ELEMENT\
	...
	if (restore_filename)
	{
		...
		{
			fprintf(stderr, "can't open file %s\n", restore_filename);
			clean_up(fp_restore);// add by zxw on 20191211
			return "";
		}
		c = fgetc(fp_restore);
		if (c == 'y')
	...
	if (restore_filename)
	{
		...
		if ((c = fgetc(fp_restore)) == 'y')
		{
			if (fscanf(fp_restore, "%lf %lf\n", &y_lower, &y_upper) != 2 ||
				fscanf(fp_restore, "%lf %lf\n", &y_min, &y_max) != 2) {
				fprintf(stderr,
					"ERROR: failed to read scaling parameters\n");
				clean_up(fp_restore);
				return "";
			}
			y_scaling = 1;
		...
		if (fgetc(fp_restore) == 'x')
		{
			...
			while (fscanf(fp_restore, "%d %lf %lf\n", &idx, &fmin, &fmax) == 3)
			{
				for (i = next_index; i<idx; i++)
					...
					{
						fprintf(stderr,
							"WARNING: feature index %d appeared in line was not seen in the scaling factor file %s. The feature is scaled to 0.\n",
							i, restore_filename);
						feature_min[i] = 0;
						feature_max[i] = 0;
					}
...
int clean_up(FILE *fp_restore) {
	if (feature_max) { free(feature_max); feature_max = nullptr; }
	if (feature_min) { free(feature_min); feature_min = nullptr; }
